param([ValidateSet('ShowMenu','InstallIISPrereqs','InstallExtensionServicePrereqs','InstallWalls','RestartAppPools','RestartSchedulerService','StartSelfMaintaining','StartFullRepair','GenerateHandoverGuide','GenerateAutoEscalation','RunDataChecks')][string]$action='ShowMenu')
...
#v1.1 - Silent Install functionality
#v1.1 - Added error handling to RestartSchedulerService
#v1.1 - Added RestartExtensionService function
...
function InstallWalls {
    # Prompt for each component that needs to be installed
    # Walls: setup.exe /verysilent /VirtualDir="Walls" /DatabaseServer="(local)" /DatabaseName="walls" /AuthType="SQL" /UserName="wb_svc" /Password="W@llBuild3r"
    # Extension Service / Scheduler Service: setup.exe /verysilent /DatabaseServer="(local)" /DatabaseName="walls" /AuthType="SQL" /UserName="wb_svc" /Password="W@llBu1ld3r"
    # API: setup.exe /verysilent /VirtualDir="APIService" /DatabaseServer="(local)" /DatabaseName="walls" /AuthType="SQL" /UserName="wb_svc" /Password="W@llBu1ld3r"
    # Team Manager: setup.exe /verysilent /VirtualDir="TeamManager" /DatabaseServer="(local)" /DatabaseName="walls" /AuthType="SQL" /UserName="wb_svc" /Password="W@llBu1ld3r"
    # Central Admin: setup.exe /verysilent /VirtualDir="CentralAdministration" /DatabaseServer="(local)" /DatabaseName="walls" /AuthType="SQL" /UserName="wb_svc" /Password="W@llBu1ld3r"
    # Activity Tracker: setup.exe /verysilent /VirtualDir="ActivityTracker" /DatabaseServer="(local)" /DatabaseName="walls" /AuthType="SQL" /UserName="wb_svc" /Password="W@llBu1ld3r" /IntermediateDatabaseServer="(local)" /IntermediateDatabaseName="ActivityTracker" /IntermediateAuthType="SQL" /IntermediateUserName="wb_svc" /IntermediatePassword="W@llBu1ld3r"
    
    Clear-Host
    Write-Host "=================== Install Walls and related components ============================================="
    Write-Host "This script will install Walls and its related components"
    Write-Host "Please answer the following questions about the database before selecting which components to install"
    Write-Host "======================================================================================================" 
    Write-Host "Database configuration" -ForegroundColor Green
    $defaultValue = "(local)"
    $prompt = Read-Host "Please enter the SQL Server database name" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    ...
    if ($config.UseSSP) { $defaultValue = "Windows" } else { $defaultValue = "SQL" }
    $prompt = Read-Host "Please enter the DB authentication type (SQL/Windows)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $AuthType = $prompt

    if ($AuthType -eq "SQL") {
        $defaultValue = $config.User
        $prompt = Read-Host "Please enter the DB user name" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $DBUser = $prompt     
        
        $defaultValue = $config.Password
        $prompt = Read-Host "Please enter the DB password" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $DBUser = $prompt     
    }

    Write-Host "----------------------------------------------"
    Write-Host "Walls installation" -ForegroundColor Green
    $defaultValue = "Y"
    $prompt = Read-Host "Do you want to install Walls? (Y/N)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $installWalls = $prompt

    if ($installWalls -eq "Y") {
        $defaultValue = "Walls"
        $prompt = Read-Host "Please enter the IIS virtual directory for Walls install" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $virtualdirwalls = $prompt
    }

    Write-Host "----------------------------------------------"
    Write-Host "Extension Service / Scheduler Service installation" -ForegroundColor Green
    $defaultValue = "Y"
    $prompt = Read-Host "Do you want to install Extension/Scheduler Service? (Y/N)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $installExtensionSvr = $prompt

    Write-Host "----------------------------------------------"
    Write-Host "API Service" -ForegroundColor Green
    $defaultValue = "Y"
    $prompt = Read-Host "Do you want to install the API Service? (Y/N)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $installAPI = $prompt

    if ($installAPI -eq "Y") {
        $defaultValue = "APIService"
        $prompt = Read-Host "Please enter the IIS virtual directory for API Service install" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $virtualdirAPI = $prompt
    }

    Write-Host "----------------------------------------------"
    Write-Host "Central Administration" -ForegroundColor Green
    $defaultValue = "Y"
    $prompt = Read-Host "Do you want to install Central Administration? (Y/N)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $installCentralAdmin = $prompt

    if ($installCentralAdmin -eq "Y") {
        $defaultValue = "CentralAdministration"
        $prompt = Read-Host "Please enter the IIS virtual directory for API Service install" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $virtualdirCentralAdmin = $prompt
    }

    Write-Host "----------------------------------------------"
    Write-Host "Team Manager" -ForegroundColor Green
    $defaultValue = "N"
    $prompt = Read-Host "Do you want to install Team Manager? (Y/N)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $installTeamMgr = $prompt

    if ($installTeamMgr -eq "Y") {
        $defaultValue = "TeamManager"
        $prompt = Read-Host "Please enter the IIS virtual directory for Team Manager install" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $virtualdirTeamMgr = $prompt
    }

    Write-Host "----------------------------------------------"
    Write-Host "Activity Tracker" -ForegroundColor Green
    $defaultValue = "N"
    $prompt = Read-Host "Do you want to install Team Manager? (Y/N)" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
    $prompt = ($defaultValue,$prompt)[[bool]$prompt]
    $installAT = $prompt
    ...
    if ($installAT -eq "Y") {
        $defaultValue = "ActivityTracker"
        $prompt = Read-Host "Please enter the IIS virtual directory for Activity Tracker install" $(If ($defaultValue -eq "") {""} Else {". Press enter to accept current value [$($defaultValue)]"}) 
        $prompt = ($defaultValue,$prompt)[[bool]$prompt]
        $virtualdirAT = $prompt
    }

    Write-Host "----------------------------------------------"
    Write-Host "Please verify" -ForegroundColor Green
    Write-Host "Install Walls: " -NoNewline
    Write-Host $installWalls -ForegroundColor Green
    Write-Host "Install Extension/Scheduler Server: " -NoNewline
    Write-Host $installExtensionSvr -ForegroundColor Green
    Write-Host "Install API Service: " -NoNewline
    Write-Host $installAPI -ForegroundColor Green
    Write-Host "Install Central Administration: " -NoNewline
    Write-Host $installCentralAdmin -ForegroundColor Green
    Write-Host "Install Team Manager: " -NoNewline
    Write-Host $installTeamMgr -ForegroundColor Green
    Write-Host "Install Activity Tracker: " -NoNewline
    Write-Host $installAT -ForegroundColor Green
    Write-Host "----------------------------------------------"
    $result = Read-Host "Please enter [Y] to begin install"

    if ($result -eq "y") {
        Write-Host "----------------------------------------------"
        if ($installWalls -eq "y") {
            Write-Host "Installing Walls...(may take several minutes)"
            #Start-Process -FilePath C:\setup.exe -ArgumentList "/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`""
            #Start-Process -FilePath C:\setup.exe -ArgumentList (/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`") -Wait
            Write-Host "Walls has been installed" 
        }
        if ($installExtensionSvr -eq "y") {
            Write-Host "Installing Extension Service & Scheduler Service...(may take several minutes)"
            #Start-Process -FilePath C:\setup.exe -ArgumentList "/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`""
            #Start-Process -FilePath C:\setup.exe -ArgumentList (/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`") -Wait
            Write-Host "Extension Service & Scheduler Service have been installed" 
        }
        if ($installAPI -eq "y") {
            Write-Host "Installing API Server...(may take several minutes)"
            #Start-Process -FilePath C:\setup.exe -ArgumentList "/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`""
            #Start-Process -FilePath C:\setup.exe -ArgumentList (/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`") -Wait
            Write-Host "API Service has been installed" 
        }
        if ($installCentralAdmin -eq "y") {
            Write-Host "Installing Central Administration...(may take several minutes)"
            #Start-Process -FilePath C:\setup.exe -ArgumentList "/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`""
            #Start-Process -FilePath C:\setup.exe -ArgumentList (/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`") -Wait
            Write-Host "Central Administration has been installed" 
        }
        if ($installTeamMgr -eq "y") {
            Write-Host "Installing Team Manager...(may take several minutes)"
            #Start-Process -FilePath C:\setup.exe -ArgumentList "/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`""
            #Start-Process -FilePath C:\setup.exe -ArgumentList (/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`") -Wait
            Write-Host "Team Manager has been installed" 
        }
        if ($installAT -eq "y") {
            Write-Host "Installing Activity Tracker...(may take several minutes)"
            #Start-Process -FilePath C:\setup.exe -ArgumentList "/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`""
            #Start-Process -FilePath C:\setup.exe -ArgumentList (/verysilent /VirtualDir=`"$virtualdir`" /DatabaseServer=`"$DBServer`" /DatabaseName=`"$DBName`" /AuthType=`"$AuthType`" /UserName=`"wb_svc`" /Password=`"W@llBuild3r`") -Wait
            Write-Host "Activity Tracker has been installed" 
        }
        Write-Host "----------------------------------------------"
        Write-Host "Installation has been completed"
    }
    else {
        Write-Host "You have chosen to cancel the installation"
    }
}

function InstallExtensionService {
    param(
        [string]$ExtServerName
    )

    $file = ExtensionServiceSetup.exe

    $session = New-PSSession -ComputerName $ExtServerName
    Copy-Item -Path $file -ToSession $session -Destination 'c:\windows\temp\ExtensionServiceSetup.exe'

    Invoke-Command -Session $session -ScriptBlock {
        c:\windows\temp\ExtensionServiceSetup.exe /verysilent /DatabaseServer="" /DatabaseName="walls" /AuthType="" /UserName="" /Password=""
    }
...
function RestartExtensionService {
...
function PrepareAutoEscalation {
    param (
...
function GenerateAutoEscalation {
    ...
    PrepareAutoEscalation -Email $input -SendEmail 1 -takeScreenshot $takeScreenshot -issue $issue
}
...
function CreateMenu {
    ...
    Write-Host "3: Press '3' to install Walls application"
    Write-Host "4: Press '4' to restart IIS App Pools"
    Write-Host "5: Press '5' to restart Scheduler service"
    Write-Host "6: Press '6' to kick off Self-Maintaining"
    Write-Host "7: Press '7' to kick off Full Repair"
    Write-Host "8: Press '8' to run data quality checks"
    Write-Host "9: Press '9' to generate Handover Guide"
    Write-Host "10: Press '10' to initiate Auto-Escalation"
